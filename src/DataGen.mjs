...
export function makeGeneratorData(galoisMultiplication) {
  const expos = [1];
...
export function makeErrorCorrectionData() {
  // simplify the format
  ...
  const generators = GENERATOR_DATA.split("|");
  ...
  const computeArray = (quality) => {
    return rows.map((row) => {
      ...
      const union =
        (numEncodableBytes << 12) | (numBlocks << 5) | generatorIndex;

      // sanity check
      if (
        numEncodableBytes !== union >> 12 ||
        numBlocks !== ((union >> 5) & 0x7f) ||
        generatorIndex !== (union & 0x1f)
      ) {
  ...
  // sorts by the bit values of L M Q H, which is ordered "MLHQ" for some reason
  return `const BLOCK_DATA = [
